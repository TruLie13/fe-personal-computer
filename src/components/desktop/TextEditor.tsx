...
export function TextEditor({ windowId, documentId }: TextEditorProps) {
  const document = useDesktopStore((state) =>
    documentId
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-win-face">
      <div className="flex items-center gap-2 border-b border-win-dark px-2 py-1">
        <button type="button" className="win-raised px-3 py-0.5" onClick={onSave}>
          Save
        </button>
      ...
      <textarea
        className="win-sunken min-h-0 flex-1 resize-none bg-white p-2 text-[12px] leading-5 outline-none"
        value={content}
        onChange={(event) => setContent(event.target.value)}
        placeholder="Write a poem or story..."
        spellCheck={false}
